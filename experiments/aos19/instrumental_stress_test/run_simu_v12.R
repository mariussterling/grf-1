...
# install.packages(packageurl, repos=NULL, type="source", lib="H/R/R-4.0.3/")
# setwd("~/github/grf-1/experiments/aos19/instrumental_stress_test")
setwd("H:/github/grf-1/experiments/aos19/instrumental_stress_test")
...
for (n in n.vals) {
  ...
  res.all = future_replicate(REPS, {
    ...
    forest.orthog = instrumental_forest(X, Y, W, Z)
    tau.forest.orthog = predict(forest.orthog, newdata = X.test)$predictions
    
    forest.plain = instrumental_forest(X, Y, W, Z, Y.hat=0, W.hat=0, Z.hat=0)
  ...
  write.table(res.all, file=paste("output_v120/out", alpha.tau, alpha.mu, sigma, confounding, axis.aligned, k.tau, k.mu, p, n, "results.csv", sep="-"), row.names=FALSE, col.names=FALSE, sep=",")
